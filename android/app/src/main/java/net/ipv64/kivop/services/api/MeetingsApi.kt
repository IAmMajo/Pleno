...
import net.ipv64.kivop.moduls.GetVotingOptionDTO
...
suspend fun getMeetings(context: Context): List<GetMeetingDTO> = withContext(Dispatchers.IO) {
...
suspend fun getMeetingsByID(context: Context, id: String): GetMeetingDTO? = withContext(Dispatchers.IO) {
  val auth = AuthController(context)
...
suspend fun getLocations(context: Context): List<GetLocationDTO> = withContext(Dispatchers.IO) {
  val auth = AuthController(context)
...
suspend fun getLocationById(context: Context, id: String): GetLocationDTO? = withContext(Dispatchers.IO) {
  val auth = AuthController(context)
