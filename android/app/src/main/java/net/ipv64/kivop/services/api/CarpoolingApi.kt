...
suspend fun postCarpoolApi(createSpecialRideDTO: CreateSpecialRideDTO): Boolean =
  withContext(Dispatchers.IO) {
    ...
    createSpecialRideDTO.description?.takeIf { it.isNotBlank() }?.let { jsonMap["description"] = it }
    createSpecialRideDTO.vehicleDescription?.takeIf { it.isNotBlank() }?.let { jsonMap["vehicleDescription"] = it }
...
suspend fun postRequestSpecialRideApi(specialRideId: String,position: Pair<Double, Double>): Boolean =
  withContext(Dispatchers.IO) {
