...
fun AlreadyVoted(navController: NavController, votingID: String) {
  val viewModel: VotingViewModel = viewModel(factory = VotingViewModelFactory(votingID)) // Use factory if needed
  ...
  Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
    ...
    Column(
      ...
      Box(modifier = Modifier.width(250.dp).height(40.dp).background(Primary_20, shape = RoundedCornerShape(8.dp))){
        Row(
          ...
          Text(text = "$votingStatus Stimmen abgegeben", color = Primary, style = TextStyles.largeContentStyle)
        }
      }
