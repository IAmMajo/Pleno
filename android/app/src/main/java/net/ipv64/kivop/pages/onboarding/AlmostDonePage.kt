...
fun AlmostDonePage(navController: NavController,initState: Int) {
  ...
  Column(
    ...
    if (state == 1){
      ...
      Text("Und warten Sie auf die Freischaltung durch den Administrator.", textAlign = TextAlign.Center)
      Button(onClick = {
        scope.launch {
      ...
      Button(onClick = {
        scope.launch {
          ...
          if (response == "Successful Login!") {
            ...
            Toast.makeText(navController.context, "Account not activated", Toast.LENGTH_SHORT).show()
          }
        }
      }) {
        Text("Bestätigt")
      ...
      Text("Warten Sie auf die Freischaltung durch den Administrator.", textAlign = TextAlign.Center)
      Button(onClick = {
        scope.launch {
          val response = checkActiv()
          if (response == "Successful Login!") {
            ...
            Toast.makeText(navController.context, getString(R.string.missing_email_verification), Toast.LENGTH_SHORT).show()
          } else {
            Toast.makeText(navController.context, getString(R.string.missing_admin_verification), Toast.LENGTH_SHORT).show()
          }
        }
      }) {
        Text("Aktualisieren")
