...
fun UserPage(navController: NavController, userViewModel: UserViewModel) {
  ...
  val topBarModifier = Modifier.zIndex(1f).padding(vertical = 12.dp, horizontal = 14.dp).height(48.dp)
  ...
  Column(modifier = Modifier.background(Primary)) {
    ...
    Column(
      ...
      if (user != null) {
        if (editMode) {
            newImgByteArray = user.name?.let { name -> ImgPicker(user.profileImage,userName = name) }?.let { uriToByteArray(navController.context, it) }
        }else {
    ...
    Column(
      modifier = Modifier
        .fillMaxWidth()
        .weight(2.5f)
        .background(Background_prime, shape = RoundedCornerShape(topStart = 22.dp, topEnd = 22.dp)) //todo: Rounded Corner anpassen
        .padding(top = 18.dp)
      ...
      IconTextField(icon = Icons.Outlined.Person, text = user?.name ?: "", edit = editMode, newText = name, onValueChange = {name = it})
      SpacerBetweenElements()
      ...
      if (editMode){
        Button(
          ...
          colors = ButtonDefaults.buttonColors(containerColor = Signal_blue, contentColor = Text_prime_light),
          onClick = {
            editMode = false
            name = ""
        ...
        Button(
          modifier = Modifier.fillMaxWidth(),
          colors = ButtonDefaults.buttonColors(containerColor = Signal_red, contentColor = Text_prime_light),
          onClick = {
