...
fun PieChart(
    ...
    Column(modifier = Modifier
        ...
        Box(modifier = Modifier
            ...
            .drawBehind {
                drawPieChart(list.sortedByDescending{ it.votes },explodeDistance = explodeDistance)
            }
...
fun DrawScope.drawPieChart(list: List<VotingResults>, explodeDistance: Float) {
    ...
    val adjustedSize = size.copy(
        width = size.width - explodeDistance*2,
        height = size.height - explodeDistance*2
    )
    ...
    for (i in list.indices) {
        ...
        if(textWidth < sliceWidth){
            ...
            drawContext.canvas.nativeCanvas.drawText(textPercentage,textPos.x,textPos.y+textPercentageOffsetY,textPaint)
        }
...
fun calcPointOnCircle(startingAngle:Float, sweepAngle:Float, explodeDistance: Float):Offset{
    ...
    //calc the x and y offset by calculation the point on a circle with the radius of the explode distance
