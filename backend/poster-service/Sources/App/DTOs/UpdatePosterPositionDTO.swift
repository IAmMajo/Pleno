...
extension UpdatePosterPositionDTO: @retroactive Content, @unchecked @retroactive Sendable {
    public func updatePosterPosition(positionId: UUID, on db: Database) async throws -> PosterPosition {
        ...
        guard let position = try await PosterPosition.query(on: db)
            .with(\.$postedBy)
            .with(\.$removedBy)
        ...
        if let newResponsibleUsers = self.responsibleUsers {
            ...
            guard let currentResponsibilities = try? await PosterPositionResponsibilities.query(on: db)
            ...
            for userId in toAdd {
                ...
                guard (try? await responsibility.save(on: db)) != nil else {
            ...
            if !toRemove.isEmpty {
                guard (try? await PosterPositionResponsibilities.query(on: db)
                    .filter(\.$poster_position.$id == positionId)
        ...
        for responsibility in position.responsibilities {
            guard (try? await responsibility.$user.load(on: db)) != nil else {
                throw Abort(.internalServerError, reason: "Error loading user for a responsibility.")
            }
            guard (try? await responsibility.user.$identity.load(on: db)) != nil else {
