...
struct MeetingController: RouteCollection {
    func boot(routes: RoutesBuilder) throws {
        let openAPITag = TagObject(name: "Meetings")
        
        let adminMiddleware = AdminMiddleware()
        let adminRoutes = routes.grouped(adminMiddleware)
        routes.get(use: getAllMeetings)
            .openAPI(tags: openAPITag, summary: "Alle Meetings abfragen", response: .type([GetMeetingDTO].self), responseContentType: .application(.json), statusCode: .ok, auth: AuthMiddleware.schemeObject)
        
        routes.get(":id", use: getSingleMeeting)
            .openAPI(tags: openAPITag, summary: "Ein Meeting abfragen", path: .type(Meeting.IDValue.self), response: .type(GetMeetingDTO.self), responseContentType: .application(.json), statusCode: .ok, auth: AuthMiddleware.schemeObject)
        
        adminRoutes.post(use: createMeeting)
            .openAPI(tags: openAPITag, summary: "Meeting erstellen", description: "Bietet sowohl das Feld locationId als auch alle Einzelfelder der Location. Beim Place eroiert das Backend automatisch, ob bereits ein Eintrag mit der Kombination plz/ort existiert. Wenn locationId gesetzt ist, werden die übrigen locationFelder ignoriert.", body: .type(CreateMeetingDTO.self), contentType: .application(.json), response: .type(GetMeetingDTO.self), responseContentType: .application(.json), statusCode: .created, auth: AdminMiddleware.schemeObject)
        
        adminRoutes.group(":id") { meetingRoutes in
            meetingRoutes.patch(use: updateMeeting)
                .openAPI(tags: openAPITag, summary: "Ein Meeting updaten", path: .type(Meeting.IDValue.self), body: .type(PatchMeetingDTO.self), contentType: .application(.json), response: .type(GetMeetingDTO.self), responseContentType: .application(.json), statusCode: .ok, auth: AdminMiddleware.schemeObject)
            
            meetingRoutes.delete(use: deleteMeeting)
                .openAPI(tags: openAPITag, summary: "Ein Meeting löschen", description: "Nur Meetings, die noch nicht begonnen haben, können gelöscht werden. Löscht ebenfalls alle vorgemerkten Anwesenheiten sowie alle zugehörigen Abstimmungen inklusive Abstimmungsoptionen.", path: .type(Meeting.IDValue.self), statusCode: .noContent, auth: AdminMiddleware.schemeObject)
            
            meetingRoutes.put("begin", use: beginMeeting)
                .openAPI(tags: openAPITag, summary: "Ein Meeting beginnen", description: "Befüllt einige Felder automatisch. Nach dem Beginn ist PATCH /meetings/{id} nicht mehr zulässig.", path: .type(Meeting.IDValue.self), response: .type(GetMeetingDTO.self), responseContentType: .application(.json), statusCode: .ok, auth: AdminMiddleware.schemeObject)
            
            meetingRoutes.put("end", use: endMeeting)
                .openAPI(tags: openAPITag, summary: "Ein Meeting beenden", path: .type(Meeting.IDValue.self), response: .type(GetMeetingDTO.self), responseContentType: .application(.json), statusCode: .ok, auth: AdminMiddleware.schemeObject)
        }
